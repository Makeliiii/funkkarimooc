...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Invert color) = inverting (rgb color)
  where
    inverting [x,y,z] = [1-x,1-y,1-z]
rgb (Mix color1 color2) = mixing (rgb color1) (rgb color2)
  where
    mixing [x1,y1,z1] [x2,y2,z2] = [(x1 + x2) / 2, (y1 + y2) / 2, (z1 + z2) / 2]
...
fromList [] = Empty
fromList (x:xs) = Pair k v (fromList xs)
  where 
    k = fst x
    v = snd x
...
toNat z = if z < 0
          then Nothing
          else Just (rekursio z)
  where
    rekursio 0 = Zero
    rekursio z = PlusOne (rekursio (z-1))
...
prettyPrint = reverse . pPrint
  where
    pPrint End = ""
    pPrint (I b) = "1" ++ pPrint b
    pPrint (O b) = "0" ++ pPrint b

fromBin :: Bin -> Int
fromBin = apustaja 0
  where
    apustaja :: Int -> Bin -> Int
    apustaja _ (O End)   = 0
    apustaja pow (I End) = (^) 2 pow
    apustaja pow (O b)   = case pow of 0 -> 0 + apustaja (pow + 1) b
                                       _ -> (^) 0 pow + apustaja (pow + 1) b
    apustaja pow (I b)   = (^) 2 pow + apustaja (pow + 1) b

toBin :: Int -> Bin
toBin 0   = O End
toBin 1   = I End
toBin num = apustaja num
  where
    apustaja 0   = End
    apustaja 1   = I End
    apustaja num = case num `mod` 2 of 0 -> O (apustaja (num `div` 2))
                                       1 -> I (apustaja (num `div` 2))
