...
allEqual [] = True
allEqual xs = apustaja xs
    where
        apustaja [] = True
        apustaja [x] = True
        apustaja (x:y:xs) = (x == y) && apustaja (y:xs)
...
distinct [] = True
distinct (x:xs) = apustaja x (x:xs) xs
    where
        pituus x = length . filter (== x)
        apustaja _ _ [] = True
        apustaja x org (y:ys) = pituus x org == 1 && apustaja y org ys
...
middle :: Ord a => [a] -> a
middle xs = sorted !! (half - 1)
    where
        sorted = sort xs
        half = if even (length xs)
...
longest = longestlist []
    where
        longestlist longest [] = longest
        longestlist longest (x:xs)
          | length longest > length x = longestlist longest xs
          | length longest == length x = if head longest < head x
                                         then longestlist longest xs
                                         else longestlist x xs
          | otherwise = longestlist x xs
...
incrementKey k  = apustaja k
    where
        apustaja _ [] = []
        apustaja k (x:xs) = if k == fst x
                            then (fst x, snd x + 1) : apustaja k xs
                            else x : apustaja k xs
...
average xs = summation / fromIntegral erobb
    where
        summation = sum xs
        erobb = length xs
...
winner scores player1 player2 = compareScores
    where
        getScore name = Map.findWithDefault 0 name scores
        compareScores = if getScore player1 >= getScore player2
                        then player1
                        else player2
...
freqs xs = apustaja newMap xs
    where
        newMap = Map.empty
        f x = case x of Nothing -> Just 1
                        Just x  -> Just (x + 1)
        apustaja mymap [] = mymap
        apustaja mymap (x:xs) = apustaja (Map.alter f x mymap) xs
...
transfer from to amount bank = if amount < 0
                               then bank
                               else apustaja
    where
        moneyGuy x = case Map.lookup x bank of Just y  -> y
                                               Nothing -> (-1)
        fromGuy    = moneyGuy from
        toGuy      = moneyGuy to
        difference = fromGuy - amount
        add x      = case x of Just x  -> Just (x + amount)
                               Nothing -> Nothing
        subtract x = case x of Just x  -> Just (x - amount)
                               Nothing -> Nothing
        apustaja   = if fromGuy == -1 || toGuy == -1 || difference < 0
                     then bank
                     else Map.alter subtract from (Map.alter add to bank)
...
swap i j arr = arr // newArr
    where
        iEle = arr ! i
        jEle = arr ! j
        newArr = [(i, jEle),(j, iEle)]
...
maxIndex arr = getIdx maxi ass
    where
        idxList = indices arr
        ass = assocs arr
        sorted = sort (map snd ass)
        maxi = last sorted
        getIdx _ [x] = fst x
        getIdx max (x:xs) = if snd x == max
                                  then fst x
                                  else getIdx max xs