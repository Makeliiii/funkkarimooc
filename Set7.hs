...
data State = Start | Error | Finished | FlourSugar | Sugar | Flour | Mixing | Baking
  deriving (Eq,Show)

step :: State -> Event -> State
step state event = case state of Start -> case event of AddEggs -> FlourSugar
                                                        _       -> Error
                                 FlourSugar -> case event of AddFlour -> Flour
                                                             AddSugar -> Sugar
                                                             _        -> Error
                                 Sugar -> case event of AddFlour -> Mixing
                                                        _        -> Error
                                 Flour -> case event of AddSugar -> Mixing
                                                        _        -> Error
                                 Mixing -> case event of Mix -> Baking
                                                         _   -> Error
                                 Baking -> case event of Bake -> Finished
                                                         _    -> Error
                                 Error -> Error
                                 _ -> Finished
...
average (x:|xs) = summation / fromIntegral erobb
  where
    toList = x : xs
    summation = sum toList
    erobb = length toList
...
reverseNonEmpty (x:|[]) = x :| []
reverseNonEmpty (x:|xs) = case toNonEmpty rv of Just xs -> xs
  where
    toList = x : xs
    rv = reverse toList
    toNonEmpty [] = Nothing
...
instance (Eq a, Ord a) => Semigroup (Set a) where
  (<>) (Set a) (Set b) = Set (sort combined)
    where
      flt [] _ new = new
      flt (x:xs) set new = if member x set
                           then flt xs set new
                           else flt xs set (x:new)
      combined = a ++ flt b (Set a) []
...
passwordAllowed pass (MinimumLength x) = length pass >= x
passwordAllowed pass (ContainsSome xs) = asdfg xs
  where
    asdfg [] = False
    asdfg (y:ys) = if length (filter (== y) pass) > 0
                   then True
                   else asdfg ys
passwordAllowed pass (DoesNotContain xs) = asdfg xs
  where
    asdfg [] = True
    asdfg (y:ys) = if length (filter (== y) pass) > 0
                   then False
