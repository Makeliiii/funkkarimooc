...
readUntil f = apustaja [] f
  where
    apustaja :: [String] -> (String -> Bool) -> IO [String]
    apustaja xs f = do
      s <- getLine
      if f s then return $ reverse xs else apustaja (s : xs) f
...
isums :: Int -> IO Int
isums n = apustaja n 0
  where
    apustaja 0 sum = return sum
    apustaja n sum = do
      i <- readLn
      print (i + sum)
      apustaja (n -1) (i + sum)
...
while :: IO Bool -> IO () -> IO ()
while cond op = apustaja cond op
  where
    apustaja cond op = do
      s <- cond
      if not s
