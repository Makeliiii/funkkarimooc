...
countValid = counter 0
  where
    valid str = str !! 2 == str !! 4 || str !! 3 == str !! 5
    counter count [] = count
    counter count (x:xs) = if valid x
                           then counter (1 + count) xs
                           else counter count xs
...
repeated [] = Nothing
repeated (x:xs) = apustaja x xs
  where
    apustaja _ [] = Nothing
    apustaja prev (x:xs) = if prev == x
                           then Just x
                           else apustaja x xs
...
sumSuccess = apustaja 0 0
  where
    apustaja 0 0 [] = Left "no data"
    apustaja rightCount count [] = Right count
    apustaja rightCount count (x:xs) = case x of Right r -> apustaja (rightCount + 1) (count + r) xs
                                                 Left l  -> apustaja rightCount count xs
...
open str (Open s) = Open s
open str (Locked s) = if str == s
                      then Open s
                      else Locked s

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Open str) = Locked str
lock (Locked str) = Locked str

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode str (Open s) = Open str
changeCode str (Locked s) = Locked s
...
compose xs ys = kulli xs ys []
  where
    kulli [] _ zs = zs
    kulli (x:xs) ys zs = case lookup (snd x) ys of Just y  -> kulli xs ys ((fst x,y):zs)
                                                   Nothing -> kulli xs ys zs
...
permute xs ys = reverse $ apustaja 0 xs ys []
  where
    apustaja idx (x:xs2) (y:ys2) zs
      | idx == x = apustaja (idx + 1) xs ys (y:zs)
      | idx == length xs = zs
      | otherwise = apustaja idx xs2 ys2 zs
