...
combine op (Picture f1) (Picture f2) = Picture f
  where
    f (Coord x y) = op (f1 (Coord x y)) (f2 (Coord x y))
...
rectangle x0 y0 w h = Shape f
  where
    f (Coord x y) = (x >= x0 && y >= y0) && (x < w + x0 && y < h + y0)
...
paintSolid color (Shape f1) (Picture f2) = Picture f
  where
    f coord = if f1 coord
              then color
              else f2 coord
