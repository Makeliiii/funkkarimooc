...
hFetchLines file = apustaja file []
  where
    apustaja file xs = do
      a <- hIsEOF file
      if not a
...
hSelectLines h nums = apustaja h 1 nums []
  where
    apustaja :: Handle -> Int -> [Int] -> [String] -> IO [String]
    apustaja _ _ [] ys = return $ reverse ys
    apustaja h i (x : xs) ys = do
      if i == x
        then do
          l <- hGetLine h
          apustaja h (i + 1) xs (l : ys)
        else do
          d <- hGetLine h
          apustaja h (i + 1) (x : xs) ys
