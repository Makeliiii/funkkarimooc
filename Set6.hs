...
instance Eq Name where
  (==) (Name name1) (Name name2) = f name1 == f name2
    where
      f [] = []
...
instance Eq a => Eq (List a) where
  ...
  (==) Empty Empty = True
  (==) (LNode a1 rest1) (LNode a2 rest2) = (a1 == a2) && (==) rest1 rest2
...
instance Eq RationalNumber where
  (RationalNumber a b) == (RationalNumber c d) = product a d == product b c
    where
      product x y = x * y
...
simplify (RationalNumber x y) = RationalNumber (x `div` yep) (y `div` yep)
  where
    yep = gcd x y
...
instance Num RationalNumber where
  (RationalNumber a b) + (RationalNumber c d) = simplify (RationalNumber (a * bMultiplier + c * dMultiplier) leastCommon)
    where
      leastCommon = lcm b d
      multiplier x = leastCommon `div` x
      bMultiplier = multiplier b
      dMultiplier = multiplier d
  ...
  signum (RationalNumber a b)
    ...
    | a < 0 && b > 0   = RationalNumber (-1) 1
    | a > 0 && b < 0   = RationalNumber 1 (-1)
    | a == 0 && b == 0 = RationalNumber 0 0
    | a == 0 && b < 0  = RationalNumber 0 (-1)
    | a == 0 && b > 0  = RationalNumber 0 1
    | a < 0 && b == 0  = RationalNumber (-1) 0
    | a > 0 && b == 0  = RationalNumber 1 0
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 clr = clr
  stepMany x clr = stepMany (x - 1) (step clr)
