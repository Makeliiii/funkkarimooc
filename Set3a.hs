...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if val1 > val2
                    then a
                    else b
...
mapMaybe2 f (Just x) (Just y) = Just (x `f` y)
...
firstHalf xs
  | size == 1 = xs
  | even size = take (size `div` 2) xs
  | otherwise = take ((size + 1) `div` 2) xs
  where
      size = length xs
...
capitalize xs = unwords mappings
      where unString = words xs
            capWord (x:xs) = toUpper x : xs
            mappings = map capWord unString
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) (powersList 0)
      where powersList a = (k^a) : powersList (a + 1)
...
joinToLength x xs = filter (\y -> length y == x) list
      where list = [y ++ z | y <- xs, z <- xs]
...
matchCommand :: [String] -> Int -> Int -> [String]
matchCommand [] _ _ = []
matchCommand (z:zs) y x = case z of "up"     -> matchCommand zs (y + 1) x
                                    "down"   -> matchCommand zs (y - 1) x
                                    "right"  -> matchCommand zs y (x + 1)
                                    "left"   -> matchCommand zs y (x - 1)
                                    "printY" -> show y : matchCommand zs y x
                                    "printX" -> show x : matchCommand zs y x

interpreter :: [String] -> [String]
interpreter commands = matchCommand commands 0 0