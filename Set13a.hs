...
split s = if ' ' `elem` s
          then Just (apustaja s [])
          else Nothing
  where
    apustaja (x : xs) f = if x == ' '
                            then (reverse f, xs)
                            else apustaja xs (x : f)
...
checkNumber (for, sur) = case (apustaja for [], apustaja sur []) of (Just x, Just y) -> Just (reverse x, reverse y)
                                                                    _                -> Nothing
  where
    apustaja :: String -> String -> Maybe String
    apustaja [] name = Just name
    apustaja (x:xs) name
      | x == '0' = Nothing
      | x == '1' = Nothing
      | x == '2' = Nothing
...
checkCapitals (for, sur) = apustaja for sur
  where
    apustaja xs ys = if toUpper (head xs) == head xs && toUpper (head ys) == head ys
                     then Just (for,sur)
                     else Nothing
...
winner scores player1 player2 = case (lookup player1 scores, lookup player2 scores) of (Just s1, Just s2) -> if s1 >= s2
                                                                                                             then Just player1
                                                                                                             else Just player2
                                                                                       _                  -> Nothing
...
safeIndex :: Num a => [a] -> Int -> Maybe a
safeIndex xs i = if i > length xs - 1 || i < 0
                 then Nothing
                 else Just (xs !! i)

selectSum :: Num a => [a] -> [Int] -> Maybe a
selectSum xs is = abuapustaja
  where
    apustaja xs is = safeIndex xs <$> is
    abustaja = apustaja xs is >>= \ys -> return ys
    abuapustaja = apuapustaja 0 abustaja
    apuapustaja sum [] = Just sum
    apuapustaja sum (x:xs) = case x of Nothing  -> Nothing
                                       (Just x) -> apuapustaja (x+sum) xs
...
countAndLog f xs = Logger (logged f xs) (length $ logged f xs)
  where
    logged f xs = map show $ filter f xs
...
bankLookup :: String -> Bank -> Maybe Int
bankLookup accountName (Bank bank) = Map.lookup accountName bank

balance :: String -> BankOp Int
balance accountName = BankOp apustaja
  where
    apustaja bank = case bankLookup accountName bank of Just x  -> (x, bank)
                                                        Nothing -> (0, bank)
...
paren x = case x of '(' -> state (\s -> ((), case s of (-1) -> negate 1
                                                       _    -> s + 1))
                    ')' -> state (\s -> ((), case s of (-1) -> negate 1
                                                       _    -> s - 1))
...
count x = state (\s -> ((), case lookup x s of Nothing -> (x, 1) : s
                                               Just y  -> apustaja x s []))
  where
    apustaja _ [] _ = []
    apustaja x (s:ss) rest = if x == fst s then (x, snd s + 1) : ss ++ rest else apustaja x ss (s:rest)
...
occurrences xs = todo
  where
    apustaja [] newState = newState
    apustaja (x:xs) newState = apustaja xs (count x)
    apuapustaja = apustaja xs (state (const ((), [])))
