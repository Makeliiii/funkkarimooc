...
count a fucktor = apustaja a listing
  where
    listing = toList fucktor
    apustaja :: (Eq a) => a -> [a] -> Int
    apustaja _ [] = 0
    apustaja a (x : xs) = if a == x then 1 + apustaja a xs else apustaja a xs
...
inBoth x y = apustaja xsList ysList
  where
    xsList = toList x
    ysList = toList y
    apustaja [] _ = []
    apustaja (x : xs) [] = apustaja xs ysList
    apustaja (x : xs) (y : ys) = if x == y then y : apustaja (x : xs) ys else apustaja (x : xs) ys
...
instance Foldable List where
  ...
  foldr f initial (LNode a rest) = f a (foldr f initial rest)
...
instance Foldable TwoList where
  ...
  foldr f initial (TwoNode a b rest) = f a (f b (foldr f initial rest))
