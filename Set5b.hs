...
treeMax node = apustaja node 0
  where
    apustaja (Node a Empty Empty) largest = if a > largest
                                            then a
                                            else largest
    apustaja (Node a l r) largest = if a > largest
                                    then max (apustaja l a) (apustaja r a)
                                    else max (apustaja l largest) (apustaja r largest)
...
allValues condition tree = apustaja tree
  where
    apustaja Empty = True
    apustaja (Node a l r) = condition a && (apustaja l && apustaja r)
...
mapTree f tree = apustaja tree
  where
    apustaja Empty = Empty
    apustaja (Node a l r) = Node (f a) (apustaja l) (apustaja r)
...
isOrdered (Node a l r) = apustaja (<a) l && apustaja (>a) r && isOrdered l && isOrdered r
  where
    apustaja _ Empty = True
    apustaja cond (Node a l r) = cond a && apustaja cond l && apustaja cond r
...
walk [StepL] (Node _ Empty _) = Nothing
walk [StepR] (Node _ _ Empty) = Nothing
walk [StepL] Empty = Nothing
walk [StepR] Empty = Nothing
walk [] (Node a _ _) = Just a
walk (step:steps) (Node a l r) = case step of StepL -> walk steps l
                                              StepR -> walk steps r
...
set path val tree = todo
  where
    apustaja [] a (Node _ Empty Empty) new = new
    apustaja (step:steps) val (Node a l r) new = case step of StepL -> apustaja steps val l (Node a l r)
                                                              StepR -> apustaja steps val r (Node a l r)
