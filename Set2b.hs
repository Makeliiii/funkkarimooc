...
leftpad str i = if length str < i then leftpad (" " ++ str) i else str
...
countdown i = "Ready! " ++ helper i ""
helper i result = if i == 0 then result ++ "Liftoff!" else helper (i - 1) (result ++ show i ++ "... ")
...
smallestDivisor n = if n `mod` 2 == 0 then 2 else kappa n 2
    where kappa n i = if n `mod` i == 0 then i else kappa n (i + 1)
...
biggestPrimeAtMost n = if isPrime n then n else checkings (n - 1)
    where checkings n = if isPrime n then n else checkings (n - 1)
